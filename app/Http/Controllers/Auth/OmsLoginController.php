...
class OmsLoginController extends Controller
{
    public function show()
    {
        if (Auth::check() && Auth::user()->role === 'oms') {
            return redirect()->route('oms.inbound.index');
        }
        return view('oms-login');
    }
    ...
    public function login(Request $r){
        ...
        if ($user && $user->role !== 'oms') {
            // hit throttle supaya tetap dihitung percobaan
            RateLimiter::hit($key, 60);
            return back()
                ->withErrors(['email' => 'Akun ini tidak memiliki akses OMS.'])
                ->withInput($r->only('email'));
        }
        ...
        $ok = Auth::attempt([
            ...
            'password'       => $r->password,
            'role'           => 'oms', // pastikan hanya WMS yang bisa lewat
        ], false);

        if ($ok) {
            RateLimiter::clear($key);
            $r->session()->regenerate();
            return redirect()->intended(route('oms.inbound.index'));
        }
    ...
    public function logout(Request $r)
    {
        ...
        return redirect()->route('oms.login')->with('ok', 'Logout berhasil.');
    }
}
